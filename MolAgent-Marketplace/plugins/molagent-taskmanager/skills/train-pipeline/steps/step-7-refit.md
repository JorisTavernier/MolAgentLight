...
uv run $PLUGIN_ROOT/skills/train-pipeline/scripts/refit_model.py \
    ...
    [--include-test-set]  # only if decisions.include_test_in_refit is true \
    --verbose
```

Include `--include-test-set` only when `decisions.include_test_in_refit` is `true` in the pipeline state. The refit script auto-detects properties from `train_info.json`.
