...
def main(**kwargs):
    """Train AutoMol classification model with custom components.
    ...
    Examples:
        ...
        uv run train_classification_advanced.py \\
            --csv-file MolagentFiles/automol_split_data.csv \\
            --custom-estimator MolagentFiles/my_classifier.py \\
            --estimator-name my_custom_clf \\
    ...
    if evaluate_after_training:
        ...
        for prop in properties:
            ...
            prop_labels = labelnames.get(prop, {})
            if prop_labels:
                # Convert class indices to labels
                class_names = [prop_labels.get(i, str(i)) for i in range(len(prop_labels))]
            else:
                class_names = None
            ...
            if eval_generate_pdf:
                try:
                    ...
                    figures = designer.show_classification_report(
                        prop=prop,
                        smiles=smiles_list_eval,
