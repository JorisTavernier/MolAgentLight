...
@click.option('--base-list', multiple=True, default=None, help='Base estimators (e.g., lr SVC rfc xgb lgbm)')
...
@click.option('--scorer', default='balanced_accuracy', type=str,
              help='Scoring metric (default: balanced_accuracy. Options: balanced_accuracy, f1_weighted, roc_auc_ovr_weighted, roc_auc_ovr, f1, accuracy, precision, recall)')
...
def main(**kwargs):
    """Train AutoMol classification model for molecular property prediction.

    This script trains an ensemble stacking model for classification tasks using nested
    ...
    if properties is None:
        # For classification, use train_properties (e.g., Class_prop5) from prep info
        # This handles quantile-binned properties that have the Class_ prefix
        properties = prep_info.get('train_properties')
        if not properties:
    ...
    # Check if quantile binning collapsed classes to single class (invalid for classification)
    # If so, fall back to original property columns which may already be categorical
    original_properties = prep_info.get('properties', [])
    properties_to_use = []
    for prop in properties:
        if prop in train_df.columns:
            n_classes = train_df[prop].dropna().nunique()
            if n_classes < 2:
                # Collapsed to single class - try original property
                orig_prop = prop.replace('Class_', '') if prop.startswith('Class_') else prop
                if orig_prop in train_df.columns and orig_prop in original_properties:
                    n_orig_classes = train_df[orig_prop].dropna().nunique()
                    if n_orig_classes >= 2:
                        if verbose:
                            print(f'WARNING: {prop} has only {n_classes} class. Using original column {orig_prop} ({n_orig_classes} classes)')
                        properties_to_use.append(orig_prop)
                    else:
                        raise ValueError(f'Property {prop} has only {n_classes} class (invalid for classification). Original {orig_prop} also has only {n_orig_classes} class.')
                else:
                    raise ValueError(f'Property {prop} has only {n_classes} class (invalid for classification).')
            else:
                properties_to_use.append(prop)
        else:
            raise ValueError(f'Property {prop} not found in dataframe columns')

    properties = properties_to_use
    if verbose:
        print(f'Final properties for training: {properties}')

    # Auto-detect or use blender properties
    ...
    if cli_blender_properties:
        # CLI overrides prep info
        blender_properties = cli_blender_properties
    else:
        # Check prep info for blender properties
    ...
    if blender_properties:
        ...
        if missing_props:
            raise ValueError(f'Blender properties not found in data: {missing_props}')
        if verbose:
    ...
    # Ensure labelnames exist for all properties we're training
    # (needed when we fallback from Class_prop to original prop)
    for prop in properties:
        if prop not in labelnames:
            if prop in train_df.columns:
                unique_values = train_df[prop].dropna().unique()
                if len(unique_values) <= 10:  # Likely a classification target
                    labelnames[prop] = {i: str(v) for i, v in enumerate(sorted(unique_values))}
    ...
    for prop in properties:
        ...
        train_info = {
            ...
            'random_state': random_state,
            'model_file': f'{prop}_stackingclfmodel.pt',
