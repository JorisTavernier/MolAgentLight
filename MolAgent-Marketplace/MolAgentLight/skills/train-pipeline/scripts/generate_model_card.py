...
def main(pipeline_state):
    ...
    if isinstance(train_info_dict, dict) and train_info_dict:
        train_info_str = ", ".join(str(v) for v in train_info_dict.values())
    else:
        train_info_str = str(train_info_dict) if train_info_dict else "N/A"

    refitted_dict = outputs.get('refitted_model_files', {})
    if isinstance(refitted_dict, dict) and refitted_dict:
        refitted_str = ", ".join(str(v) for v in refitted_dict.values())
    else:
        refitted_str = str(refitted_dict) if refitted_dict else "N/A"
...
For Docker, GPU, and production deployment, refer to the AutoMol deployment documentation.
...
| Model file | `{model_file}` |
| Training info | `{train_info_str}` |
| Refitted model | `{refitted_str}` |
