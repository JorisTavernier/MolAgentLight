...
@click.option('--model-config', default=None, type=click.Choice(['single_method', 'inner_methods', 'inner_stacking', 'single_stack', 'top_method', 'top_stacking']),
...
@click.option('--scorer', default='r2', type=str,
              help='Scoring metric (default: r2. Uses regression scorers on binary classification targets)')
...
def main(**kwargs):
    """Train AutoMol regression-classification model with custom components.

    This script trains an ensemble stacking model that uses regression methods on
    binary classification targets (RegressionClassification task). This approach
    can be useful for binary classification problems where regression-based
    ...
    Examples:
        ...
        uv run train_regression_classification_advanced.py \\
    ...
    if labelnames is not None:
        ...
        for prop in properties:
            if prop in train_df.columns:
                unique_values = sorted(train_df[prop].dropna().unique())
                if len(unique_values) == 2:  # Binary classification
                    labelnames[prop] = {0: str(unique_values[0]), 1: str(unique_values[1])}
    ...
    # Task is RegressionClassification for this script
    ...
    # Add custom estimators to method archive (use regressor archive for RegressionClassification)
    ...
    out = stacked_model.predict(props=None, smiles=smiles_list, compute_SD=True)
    ...
    for prop in properties:
        ...
        # Model file path (use stackingregmodel even for RegressionClassification)
    ...
    if evaluate_after_training:
        ...
        for prop in properties:
            ...
            # Generate evaluation report (use regression report for RegressionClassification)
